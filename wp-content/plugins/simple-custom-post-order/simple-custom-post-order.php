...
class SCPO_Engine {
    ...
    function _check_load_script_css() {
        ...
        if (isset($_GET['orderby']) || strstr($_SERVER['REQUEST_URI'], 'action=edit') || strstr($_SERVER['REQUEST_URI'], 'my-admin/post-new.php'))
            return false;
        ...
        if (!empty($objects)) {
            ...
            if (!isset($_GET['post_type']) && strstr($_SERVER['REQUEST_URI'], 'my-admin/edit.php') && in_array('post', $objects)) { // if post
                $active = true;
            }
        }
