...
if ( interface_exists( 'RWMB_Storage_Interface' ) ) {
	...
	class RWMB_Table_Storage implements RWMB_Storage_Interface {
		...
		public function get( $object_id, $meta_key, $args = false ) {
			...
			return empty( $row[ $meta_key ] ) ? $default : maybe_unserialize( $row[ $meta_key ] );
		}
		...
		public function row_exists( $object_id ) {
			$count = $this->wpdb->get_var( $this->wpdb->prepare( "SELECT COUNT(*) FROM {$this->table} WHERE `ID` = %d", $object_id ) );
		...
		public function update_row( $object_id, $row ) {
			...
			return $this->wpdb->update( $this->table, (array) $row, array(
				'ID' => $object_id,
			) );
		}
		...
		public function insert_row( $object_id, $row ) {
			...
			return $this->wpdb->insert( $this->table, $row );
		}
		...
		public function delete_row( $object_id ) {
			return $this->wpdb->delete( $this->table, array(
				'ID' => $object_id,
			) );
		}
	}
